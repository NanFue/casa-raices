 ...
 *
 * Provides the module configuration fields. This is delegated to its own class 
...
class FormBuilderConfig extends Wire {
	...
	public function __construct(array $data) {
		...
		if(!is_file($config->paths->templates . FormBuilderMain::name . '.php')) {
			$out = "Please copy " . 
				$config->urls('FormBuilder') . FormBuilderMain::name . ".php to " . 
				$config->urls->templates . FormBuilderMain::name . '.php'; 
			$this->error($out);
		}
	...
	public function getConfig() {
		...
		if(version_compare(wire()->config->version, self::requiredVersion, '<')) {
			$this->error(
				"Form Builder requires ProcessWire " . self::requiredVersion . " or newer. " . 
				"You need to update your ProcessWire version before using Form Builder."
			); 
		}
	...
	protected function configEmbeds() {
		...
		$f->description = $this->_('Check all fields that are allowed to have forms embedded in them using the easy-embedding method. Easy embedding enables you to type a special embed "tag" on its own line and have the form automatically inserted there when the page is viewed. We highly recommend that you use easy embedding, however if you won’t be using it, you may leave all unchecked.');
	...
	protected function configLicense() {
		...
		if($input->post('licenseKey') && $input->post('licenseKey') != $session->get('FormBuilderLicenseKey')) {
			...
			if($result === 'valid') {
				...
				$licenseKey = '';
				$infos = array();
				if($result) $infos[] = "result=$result";
				if($error) $infos[] = "error=$error";
				if(method_exists($http, 'getLastSendType')) $infos[] = "type=" . $http->getLastSendType(); 
				$info = count($infos) ? "(" . implode(', ', $infos) . ")" : "";
				$f->error("Unable to complete http connection to validate product key, please try again or email ryan@processwire.com for help. $info"); 
			}
		}
	...
	protected function configMarkup() {
		...
		$parentFieldset = $modules->get('InputfieldFieldset');
		$parentFieldset->label = $this->_('Advanced Output Settings');
		$parentFieldset->description = 
			$this->_('We suggest leaving all these settings as they are, unless you have a specific need to change them.') . ' ' . 
			$this->_('Many of these settings are for legacy purposes and many not likely to be useful except in older installations.');
		$parentFieldset->notes = 
			$this->_('Because some of these settings involve configuration of HTML markup, servers with mod_security may interfere with saving them. This is why they are disabled by default.');
		...
		$f->name = 'advconfig';
		$f->label = $this->_('Do you want to modify the advanced output settings?'); 
		$value = empty($this->data['advconfig']) ? 0 : 1;
		$f->attr('value', $value);
		if($value) {
			$f->description = $this->_('After disabling this, save. Any changes to your settings will remain even when this toggle is off.');
			$parentFieldset->collapsed = Inputfield::collapsedNo;
		} else {
			$f->description = $this->_('After enabling this, save and return here to view or modify the settings.');
		}
	...
	protected function upgradeChecks() {
		...
		$permission = $this->wire()->permissions->get('form-builder-add');
