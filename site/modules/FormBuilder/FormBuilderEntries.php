 ...
 * @method array importCSV($filename, array $options = array())
...
class FormBuilderEntries extends Wire {
	...
	protected function databaseVersion() {
		...
		if(!method_exists($database, 'getVersion')) return '0';
		return $this->database->getVariable('version', true, false);
	}
			...
			if(strpos($key, '_errors') === 0 && empty($value) && preg_match('/^_errors\d+$/', $key)) {
				...
				if($field && $field->type == 'FormBuilderFile') {
					foreach($value as $k => $file) {
						...
						if(is_array($file)) {
							if(is_file($file['file'])) $data[$key][$k]['file'] = basename($file['file']); 
						} else {
	...
	protected function buildFindQuery($selectorString, $allForms = false) {
		...
		foreach($selectors as $selector) {	
			...
			if($field === 'data') {
				// search all fields
				$this->buildFindQueryData($query, $operator, $value, $selector->not);
				continue;
			} else if(!in_array($field, $systemFields)) {
				// handle custom field encoded within data
				$this->buildFindQueryField($query, $field, $operator, $value, $allForms, $selector->not);
				continue;
	...
	protected function buildFindQuerySort(DatabaseQuerySelect $query, $value, array &$sorts) {
		...
		$f = $this->getForm()->getFieldByName($value);
		...
		if(!$this->mySqlVersion('5.7.0')) {
			$this->error("MySQL version does not support sort by field: $value");
	 ...
	 * 
	 */
	protected function buildFindQueryField(DatabaseQuerySelect $query, $field, $operator, $value, $allForms = false, $not = false) {

		$database = $this->wire()->database;
		if(!$allForms) {
			$formField = $this->getForm()->getFieldByName($field);
	...
	public function find($selectorString, array $options = array()) {
		...
		if($exception) {
			...
			if(strpos($msg, 'JSON') === false) throw $exception;
			if(version_compare($this->mySqlVersion(), '5.7.0', '>=')) throw $exception;
			$this->error($this->_('Searching or sorting within specific form entry fields requires MySQL 5.7.0 or newer')); 
		}
	 ...
	 * @param array $options
	 * @throws FormBuilderException|\PDOException|WireException
	 * @return array
	 * @since 0.5.2
	 * 
	 */
	public function ___importCSV($filename, array $options = array()) {
		...
		$form = $this->getForm();
		$fp = fopen($filename, 'r');
		if($fp === false) throw new FormBuilderException("Cannot open CSV file: $filename"); 
		$cols = fgetcsv($fp); // first row is column names
		
		foreach($cols as $num => $name) {
			// validate that $name is valid for form	
			if(in_array($name, $this->reservedProperties)) {
				// ok: reserved entry property
			} else if($form->hasField($name)) {
				// ok: form field name
			} else {
				// unknown column
				$result['warnings'][] = "Unrecognized column: $name";
				continue;
			}
			$cols[$num] = trim($name);
		}
		...
		while(($row = fgetcsv($fp)) !== false) {
			...
			$isEmptyRow = true;
			foreach($cols as $num => $name) {
				if(!isset($row[$num])) continue;
				$value = trim($row[$num]);
				$entry[$name] = $value;
				if(!empty($value)) $isEmptyRow = false;
			}
			if($isEmptyRow) {
				$result['linesSkipped'][] = $lineNum;
				continue;
			}
			$id = $this->save($entry);
			if($id) {
				...
				// error adding entry (not likely)
				$result['linesFailed'][] = $lineNum;
			}
		}
		
		fclose($fp);
		
		$result['numSkipped'] = count($result['linesSkipped']);
		$result['numFailed'] = count($result['linesFailed']); 
	...
	public function ___exportCSV(FormBuilderForm $form, $selectorString, array $options = array()) {
		...
		while($row = $query->fetch(\PDO::FETCH_ASSOC)) { 
			...
			if($row['flags'] & self::flagSpam) $flags[] = 'spam';
			...
			foreach($fields as $name) {
				...
				if($formBuilderField) {
					$value = $this->exportCSV_formatValue($form, $formBuilderField, $value, $entry);
		...
		if($field->type == 'Datetime' && $value) {
			...
			foreach($value as $k => $v) {
				...
				if(basename($file) === $file) {
					$file = $this->getFilesPath($entry['id'], false) . $file;
				}
				
				$fileURL = $forms->getFileURL($form->id, $entry['id'], $file);
				if($fileURL === false) {
					unset($value[$k]); // file not found
				} else {
					$value[$k] = $fileURL . ($desc ? "\n$desc" : "");
				}
			}
	...
	protected function flag($flag) {
		...
		if($flag === 'spam') return self::flagSpam;
		return 0;
	}
	...
	public static function _install($database) {
		...
		$sql = str_replace(array('{engine}', '{charset}'), array($engine, $charset), $createTable);
		$fail = false;
		...
		if($fail && strtolower($engine) !== 'myisam') try {
			// try again using MyISAM as engine
			$sql = str_replace(array('{engine}', '{charset}'), array('MyISAM', $charset), $createTable);
			$database->exec($sql);
			$fail = false;
		} catch(\Exception $e) {
			$fail = $e;
		...
		if($fail && strtolower($charset) !== 'utf8') try {
			// try again using UTF8 charset 
			$sql = str_replace(array('{engine}', '{charset}'), array('MyISAM', 'utf8'), $createTable);
			$database->exec($sql);
			$fail = false;
		} catch(\Exception $e) {
		...
		return $fail ? false : self::entriesTable;
	}
	 ...
	 * 
	 */
	protected static function _hasColumnType($database, $column, $type = '') {
		$table = self::entriesTable;
		$query = $database->prepare("SHOW COLUMNS FROM $table LIKE :column");
		$query->bindValue(':column', $column);
		$query->execute();
		if(!$query->rowCount()) return false;
		$row = $query->fetch(\PDO::FETCH_ASSOC); 
		if($type && strtolower($row['Type']) !== strtolower($type)) return false;
		return $type;
	}
	...
	public static function _upgrade($database, $verbose) {
		...
		if(!self::_hasColumnType($database, 'created', 'DATETIME')) {
			$database->exec("ALTER TABLE $table MODIFY `created` DATETIME NOT NULL");
			if($verbose) $database->message("Updated $table.created type to DATETIME"); 
		}
	
		if(!self::_hasIndex($database, 'created_forms_id')) {
			$database->exec("ALTER TABLE $table ADD INDEX created_forms_id (created, forms_id)");
			if($verbose) $database->message("Added FormBuilder index $table.created_forms_id");
		}
	 ...
	 * 
	 */
	public function mySqlVersion($testVersion = '', $testOperator = '>=') {
		if($this->mySqlVersion === 0) {
			$query = $this->database()->query("SELECT VERSION()");
			$query->execute();
			$this->mySqlVersion = $query->fetchColumn();
			if(stripos($this->mySqlVersion, 'MariaDB')) {
				list($version, /* MariaDB */) = explode('-', $this->mySqlVersion);
				// substitute MySQL version for MariaDB
				if(version_compare($version, '10.2.3', '>=')) {
					$this->mySqlVersion = '5.7.0';
				} else {
					$this->mySqlVersion = '5.6.4';
				}
			}
		...
		if($testVersion) {
			return version_compare($this->mySqlVersion, $testVersion, $testOperator); 
		}
