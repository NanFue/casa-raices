 ...
 * @property int $flags Form flags (see FormBuilderProcessor::formFlag* constants)
 * @property int $formFlags Alias of $flags
 ...
 * 
 * @property int $saveFlags Bitwise flags for save actions (see FormBuilderProcessor::saveFlag* constants)
 ...
 * @property array|null $spamWords
 * @property int|null $spamFlags Spam flags (0=none, 2=silent, 4=verbose, 8=spam entry)
 ...
 * @property string $savePageSubfields Textarea with "form_field=page_field" or "form_field=page_field.subfield" one per line. 
 ...
 * @property int $spamEntryDays Maximum days a spam entry is allowed to be saved in the system
 * 
 * 
 */
...
class FormBuilderForm extends FormBuilderField {
	...
	public function get($key) {
		if($key === 'formFlags') $key = 'flags'; // alias
		switch($key) {
			case 'form':
			case 'fbForm':	
				$value = $this;
	 ...
	 * @param string $key
	 * @param mixed $value
	 * @return FormBuilderData|FormBuilderField
	 * 
	 */
	public function set($key, $value) {
		...
		} else if($key === 'emailFiles') { 
			// emailFiles is legacy property moved to flags in FormBuilder v50
			$method = (int) $value ? 'addFlag' : 'removeFlag';
			$this->$method(256); // FormBuilderProcessor::formFlagEmailFiles
			return $this;
		}
	 ...
	 * Get form Framework
	 * 
	 * @return null|FormBuilderFramework
	 * 
	 */
