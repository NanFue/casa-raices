...
class FormBuilderMain implements \IteratorAggregate, \Countable {
	 ...
	 *
	 */
	const RCD = 'ProcessWire Form Builder - Copyright 2021 by Ryan Cramer Design, LLC';
	...
	/**
	 * Default/fallback jQuery UI theme
	 * 
	 */
	const defaultTheme = 'delta';

	/**
	 * Default/fallback framework
	 * 
	 */
	const defaultFramework = 'Basic';
	...
	static public function parseFilesPath($path) {
		...
		if(!is_dir($path)) {
			$path = wire()->config->paths->cache . self::name . '/';
		}
	...
	public function save(FormBuilderForm $form) {
		$name = $this->sanitizeFormName($form->name);
		if($name != $form->name) $form->name = $name;
		$id = (int) $form->id; 
		$data = $form->getArray();
		unset($data['name'], $data['type'], $data['id']);
		$table = self::formsTable;
		$sql = ($id ? "UPDATE" : "INSERT INTO") . " $table SET name=:name, data=:data " . ($id ? "WHERE id=:id" : ''); 
		$query = $this->database->prepare($sql); 
	...
	public function addNew($formName) {
		$formName = $this->sanitizeFormName($formName);
		$table = self::formsTable;
		$query = $this->database->prepare("INSERT INTO $table SET name=:name, data=''");
		$query->bindValue(':name', $formName); 
		$query->execute();
		$form = new FormBuilderForm($this);
	 ...
	 * 
	 */
	protected function sanitizeFormName($formName) {
		if(strlen($formName)) {
			$formName = preg_replace('/[^-_.a-z0-9]/i', '-', $formName);
			if(ctype_digit($formName) || !preg_match('/[-_a-z]/i', $formName)) {
				// form must have at least one a-z in it
				$formName = trim("form-$formName", '-');
			}
		}
		if(!strlen($formName)) $formName = 'form-' . time();
		return $formName;
	}
	...
	public function delete(FormBuilderForm $form) {
		...
		try {
			...
			$table = self::formsTable;
			$query = $this->database->prepare("DELETE FROM $table WHERE id=:id LIMIT 1");
	...
	public function formName($formName, $throw = true) {
		...
		if($name === '' && $throw) {
			throw new FormBuilderException("Unable to find form name: $formName");
		}
	...
	public function exportJSON($id) {
		$id = $this->formID($id);
		$table = self::formsTable;
		$query = $this->database->prepare("SELECT data FROM $table WHERE id=:id");
		$query->bindValue(':id', $id, \PDO::PARAM_INT); 
		$query->execute();
	...
	public function importJSON($id, $json) {
		...
		if($data && array_key_exists('children', $data)) {
			// JSON is valid
			$table = self::formsTable;
			$query = $this->database->prepare("UPDATE $table SET data=:json WHERE id=:id"); 
	...
	public function getFileKey($formID, $entryID, $filename) {
		if(basename($filename) === $filename) $filename = $this->getFilePath($formID, $entryID, $filename);
	...
	public function getFileFromKey($key) {
		...
		if($form->hasFlag(256)) { // 256=FormBuilderProcessor::formFlagEmailFiles
			/** @var User $user */
			$user = $this->wire('user');
	 ...
	 * 
	 */
	public function getFilePath($form, $entryID, $filename) {
	...
	public function viewFile($key) {
		...
		$forceDownload = strpos($contentType, '+') === 0;
		...
		header("Content-Type: " . ltrim($contentType, '+'));
	...
	public function helperPath($for, $item = '') {
		...
		if($item) {
			...
			if($for === 'themes') return $_path . self::defaultTheme . '/';
			$this->error("Unable to locate directory for $for '$item'");
			return $_path . "$item/";
		}
	...
	public function helperURL($for, $item = '') {
		...
		// attempt to locate in /site/modules/FormBuilder/themes/
		if($item) {
			$url = $this->config->urls->get('FormBuilder');
			$path = $this->config->paths->get('FormBuilder') . "$for/$item/";
			if(!is_dir($path)) {
				// if theme can't be found even here, substitute default theme
				if($for === 'themes') return $url . "$for/" . self::defaultTheme . '/';
				$this->error("Unable to locate $for directory for '$item'");
			}
			return $url . "$for/$item/";
		}
	 ...
	 * @param bool $fallback Fallback to basic framework if requested one does not exist?
	 * @return FormBuilderFramework|null
	 *
	 */
	public function getFramework($form, $fallback = true) {
		
		static $frameworks = array();
		
		$form = $this->form($form);
		if(!$form->framework) $form->framework = self::defaultFramework;
		...
		if(!file_exists($file)) {
			if($fallback) {
				...
				if($form->framework == 'bootstrap') {
					// older versions of bootstrap framework fallback to bootstrap5
					$class = 'FormBuilderFrameworkBootstrap5';
				} else {
					$class = 'FormBuilderFramework' . self::defaultFramework;
				}
				
				$nsClass = wireClassName($class, true);
				$file = $this->frameworksPath() . "$class.php";
				if(!file_exists($file)) {
					// fallback to /site/modules/FormBuilder/frameworks/FormBuilderFrameworkBasic.php
					$file = $this->config->paths->get('FormBuilder') . "frameworks/$class.php";
				}
			} else {
				return null;
			}
	...
	public function error($str) {
		$forms = $this->wire('forms');
		if(!$forms) $forms = wire('modules')->get('FormBuilder');
		if(!$forms) $forms = $this->wire;
		if($forms) $forms->error($str);
	}
	 ...
	 *
	 * @param string $key
	 * @return WireDatabasePDO|Config|null
	 * 
	 */
	 ...
	 * Install the tables
	 * 
	 * @deprecated (now done by FormBuilderInstall)
	 *
	 */
