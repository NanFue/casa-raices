...
class FormBuilderMaker extends Wire {
	...
	// submitType: error occurred, show form again for retry
	const submitTypeRetry = 4;
	
	// submitType: duplicate form submission
	const submitTypeDuplicate = 5;
	...
	public function __get($name) {
		if($name === 'formName') return $this->processor->formName;
	...
	public function getSubmitType() {
		...
		if($valid === self::submitTypeFinal) { 
			$valid = $this->processor->validSubmitKey(array('getErrorName' => true)); 
			if($valid === true) {
				// ok
			} else if($valid === 'wrong-key') {
				// wrong-key
				$valid = self::submitTypeRetry;
			} else if($valid === 'duplicate') {
				// duplicate
				$valid = self::submitTypeDuplicate;
			} else {
				// key-missing, key-short, wrong-form, wrong-qty, missing-0
				$valid = false;
			}
		}
