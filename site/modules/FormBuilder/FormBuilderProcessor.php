 ...
 * @method string processInputIsSpam(InputfieldForm $form) Checks for spam and returns name of spam filter that was triggered, or blank string if not spam.
...
class FormBuilderProcessor extends WireData {
	 ...
	 * 
	 */
	protected function ___renderOrProcessReady($form, $submitType) { }
	...
	protected function ___processInput($id, $submitType) {
		...
		if(($this->saveFlags & self::saveFlagFilterSpam) && $this->honeypot) {
			// honeypot spam filter intentionally before input is processed (optimization)
			if(strlen($input->post($this->honeypot))) {
				$this->processInputSpammed($this->form, 'honeypot');
				if(!($this->spamFlags & self::spamFlagEntry) && !($this->spamFlags & self::spamFlagVerbose)) return true;
			}
		}
		...
		if($isFinalSubmit) {
			$spam = $this->processInputIsSpam($this->form); 
			if($spam) {
				$this->processInputSpammed($this->form, $spam);
				if($this->spamFlags & self::spamFlagEntry) {
	 ...
	 * 
	 */
	protected function ___processInputIsSpam(InputfieldForm $form) {
		return ($this->saveFlags & self::saveFlagFilterSpam ? $this->spammer()->isSpam($form) : '');
	}
	...
	public function validSubmitKey($options = array()) {
		...
		if($submitKey === null || !strlen($submitKey)) {
			...
			$this->addError(print_r($a, true) . ' ... ' . $sessionKeyLast);
			$error = 'wrong-key';
		}
	...
	public function entries() {
		...
		if($fbForm) {
			...
			return $forms->get($this->formName)->entries();
		}
	}
	...
	public function ___emailFormResponder(InputfieldForm $form, array $data) {
		...
		if(!strlen($this->responderTo)) return 0;		
		...
		$this->emailFormPopulateSkipFields($email, $form);
		if(!$this->emailFormResponderReady($form, $email)) return 1;
		...
		foreach(explode(',', $this->responderTo) as $fieldName) {
			...
			if(!$field) continue;
			$responderTo = $sanitizer->email($field->attr('value'));
	...
	protected function ___postAction2(array $data) {
		...
		$headers = array(
			...
			'User-Agent' => 'ProcessWire FormBuilder/4 (+https://processwire.com)'
		);
