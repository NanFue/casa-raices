...
class FormBuilderRender extends WireData {
	 ...
	 * 
	 */
	public function __construct(FormBuilder $forms, FormBuilderForm $form, array $vars = array()) {
		parent::__construct();
		$forms->wire($this);
		$this->forms = $forms;
		$this->form = $form;
	...
	public function ___renderInlineStyles($wrap = true, $extras = true) {
		$styles = '';
		if($extras && $this->wire()->page->template->name === 'form-builder') {
			// embed method A or B
			$styles .=
	...
	protected function checkTemplateVersion(&$out) {
		...
		if($forms->getTemplateVersion() < FormBuilder::requireTemplateVersion) {
			$page = $this->wire()->page;
			if($page->template->name === 'form-builder') {
				$error = 
					"<strong>This template file (/site/templates/form-builder.php) is out of date.</strong> " . 
					"Please replace it with a new copy from /site/modules/FormBuilder/form-builder.php. " . 
					"This error message only visible to administrators.";
				if($page->editable()) {
