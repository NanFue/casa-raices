...
function ProcessFormBuilder() {
	...
	function setupEntries() {
		...
		// Setup entries action buttons
		var $action = $('#Inputfield_submit_action_entries_dropdown_value');
		if($action.length) {
			// PW 3.0.180+
			$('#entries_list_form').on('submit', function() {
				var $form = $(this);
				var action = $action.val();
				
				if(!action.length) return;
				if($form.hasClass('pwfb-action-confirmed')) return true;
				
				var $ul = $('#Inputfield_submit_action_entries_dropdown');
				var $a = $ul.find('a[data-pw-dropdown-value=' + action + ']');
				var numChecked = $form.find('input[name="checked_entries[]"]:checked').length;
				
				if(numChecked) {
					var confirmText = numChecked === 1 ? $form.attr('data-confirm-singular') : $form.attr('data-confirm-plural');
					confirmText = confirmText.replace('%s', $a.text()).replace('%d', numChecked);
					ProcessWire.confirm(confirmText, function() {
						$('#confirm_action_entries').val(action);
						$form.addClass('pwfb-action-confirmed').submit();
					});
				} else {
					ProcessWire.alert($form.attr('data-none-checked'));
				}
			...
			// PW 3.0.179 or prior
			$(document).on('click', '.pwfb-confirm-submit', function() {
				var $button = $(this);
				var $form = $button.closest('form');
				
				if(!$form.length) return;
				if($button.hasClass('pwfb-action-confirmed')) return true;
				
				var numChecked = $form.find('input[name="checked_entries[]"]:checked').length;
				
				if(numChecked) {
					var confirmText = numChecked === 1 ? $form.attr('data-confirm-singular') : $form.attr('data-confirm-plural');
					confirmText = confirmText.replace('%s', $button.text()).replace('%d', numChecked);
					ProcessWire.confirm(confirmText, function() {
						$button.addClass('pwfb-action-confirmed').click();
					});
				} else {
					ProcessWire.alert($form.attr('data-none-checked'));
				}
	...
	function setupEntriesSearch() {
		...
		$filterFieldset.find(':input[required]').removeAttr('required');
