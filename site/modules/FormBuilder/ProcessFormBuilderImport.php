...
/**
 * ProcessWire Form Builder: Import
 *
...
class ProcessFormBuilderImport extends Wire {
	...
	public function execute() {
		...
		if($input->post('submit_import_commit')) {
			...
			$importProperties = $input->post('_import_properties');
			if(!is_array($importProperties)) $importProperties = array();
			if(in_array('sort', $importProperties)) {
				$order = explode(',', (string) $input->post('_import_sort'));
